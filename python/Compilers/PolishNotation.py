...
def splitNote(s, operand=[], operator={}):
	# set defulat value of operand if operand is empty
	if (len(operand) == 0) :
		operand = map(chr, range(65, 91)) + map(chr, range(97, 123)) + map(chr, range(48, 58)) + ["_"]
...
def InfixToPostfix(infix, operand=[], operator={'+':[1, 2], '-':[1, 2], '/':[2, 2], '*':[2, 2]}):
	...
	for i in range(0, len(infix)) :
		...
		# operator found, pop till operator of lower precedence found
			...
			# binary operator
			if operator[infix[i]][1] == 2 :
				if len(stack) == 0 :
					stack.append(infix[i])
				else :
					prec = operator[infix[i]][0]
